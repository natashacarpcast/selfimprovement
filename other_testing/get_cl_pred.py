...
df = spark.read.csv('../clean_mfd2+liwc.csv', header=True)
...
pca_kmeans = KMeans(k=4, seed=2503)
...
print("Silhouette with squared euclidean distance = {} when using PCA and 4 k clusters".format(str(silhouette)))
print("Predictions corresponding to PCA and 4 k clusters")
...
merged_df.filter(F.col('prediction') == 3) \
                    .show(10)

#Send to parquet files
merged_df.write.mode('overwrite').parquet('predictions_4k')
